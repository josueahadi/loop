...
> **Onboarding reality (verified July 2026).** Flutterwave's **self-serve** signup currently offers **Nigeria as the only selectable country**; other countries — Rwanda included — are routed to a **"Contact Sales"** enterprise lead form (which asks for annual USD processing volume and business-registration status). So although Flutterwave's marketing lists Rwanda as supported and *processes* Rwandan cards/MoMo, a Rwandan business **cannot obtain test/live merchant keys self-serve** — it requires a sales conversation and business verification. For this project the demo therefore runs on **`PAYMENT_DRIVER=stub`**, which exercises the entire pass-through architecture (checkout → pending → webhook → Paid ✓, owner/participant guards, webhook idempotency) with no credentials. The `flutterwave` driver is **code-complete and env-swappable**: the steps below apply once test keys are obtained via Flutterwave sales (or once self-serve Rwanda onboarding opens). If a Rwanda-native, self-serve gateway is preferred later (e.g. a local MoMo aggregator), it slots behind the same `PaymentProvider` interface without touching the API contract or the mobile flow.

1. **Get test keys.** Create a Flutterwave account, switch the dashboard to **Test mode**, and copy the **Public**, **Secret**, and **Encryption** keys (Settings → API Keys). Flutterwave (not Stripe) because Stripe has **no Rwanda-merchant support at all**; Flutterwave settles **RWF card + MTN Mobile Money** natively — subject to the onboarding note above.
...
> **When this was found.** Discovered during on-device testing on **10 July 2026** (the timeout was reproduced on MTN mobile data at ~19:56–19:59, visible in the status bar of `screenshots/raw-error-login-before-fix.jpeg` and `raw-error-register-before-fix.jpeg`), in the final stretch before the defence. It surfaced late because it only appears on specific mobile-data paths (the emulators, the desktop dev network, and Wi-Fi all reached the API normally), so nothing earlier exposed it. It is called out here and in the report so it is fixed, and understood, rather than mistaken for a demo-day app failure.
