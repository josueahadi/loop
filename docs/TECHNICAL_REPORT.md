...
The response is a focused minimum viable product covering the matching, communication, and trust core: role-based onboarding with mandatory driver verification, a map-based real-time matching view with vehicle-type filtering, a transparent system-suggested price that the owner sets rather than the platform, in-app messaging, in-app turn-by-turn navigation for the driver (with a hand-off to the driver's own maps app as a secondary option), and a two-way rating system that builds portable reputation. In-app payment is provided as a **pass-through** — the platform initiates the settlement through a provider and records the verified outcome, but never holds, escrows, or takes a commission on the funds — which keeps Loop lightweight and avoids the working-capital exposure that undermined heavier regional platforms. Settling off-platform remains valid; paying in-app is offered, never forced.
...
**Specific objective 2: design and develop the MVP across iterative cycles.** Fully addressed. The MVP was built in milestones (M1 to M8, plus an M3.5 location increment), each delivering a working slice: foundation and auth, matching, pricing and jobs, location and geocoding, the transaction loop, trust and ratings, and the admin console. A later M7 increment added road routing, a distance-and-time (later weight-aware) pricing rule, and in-app turn-by-turn navigation; M8 added in-app pass-through payments (see [Section 5](#5-discussion)). Every functional requirement is implemented ([Section 4.1](#41-functional-requirements-a-traceability-view)).
...
| FR5 | System-suggested pricing, configurable and owner-editable | Implemented | Distance-and-time formula over DB-held config (road distance/duration from the OSRM routing proxy, great-circle fallback), with the estimate and the owner's price stored separately (`pricing.service`, `routing.service`, `job` entity). |
...
**The pricing rule is rule-based and transparent.** The suggested price (v3) combines a per-vehicle-type floor, a base fare, a distance term, a time term, and a weight term, all scaled by a load-size factor:
...
It is computed in the pricing service and rounded to whole Rwandan francs (the currency has no minor unit, so amounts are integers and are never multiplied by a hundred). The `min_fare` floor sits inside the `max()`, so the size factor scales the floor too, and a very short trip still covers a minimum. The distance and duration come from road routing (an OSRM proxy); when the router is unavailable the estimate falls back to the PostGIS great-circle distance, drops the time term, and flags the source, so an estimate is never blocked on a third party (the weight term is likewise dropped when weight is unknown). The load-size factor reflects the load's **bulk/awkwardness** and is independent of weight, which is priced separately via `rate_per_kg` — so a heavy dense load and a light bulky load are each priced on the dimension that drives the cost. Every parameter is read from a configuration table seeded from requirements data, not hard-coded, so it can be corrected without a redeploy. (The weight term and the recalibrated rates were added as pricing v3 in response to supervisor feedback that the earlier placeholder fares were unrealistically low and that weight did not influence price; see `docs/CHANGES_LOG.md`.)

The seeded v3 parameters (placeholders pending field research, all in whole RWF) are:
...
The load-size factor is `1.0` for small, `1.3` for medium, and `1.6` for large. As a worked example, a pickup carrying a medium load of 150 kg over a ~4 km / 10 min road trip is `max(3000, 2500 + 550×4 + 50×10 + 8×150) × 1.3 = max(3000, 6400) × 1.3 ≈ 8,320 RWF`.

The figure is a suggestion: the estimate and the owner's chosen price are stored as separate fields, and the owner sets the final price. The distance, duration, and distance-source used are also persisted on the job, as inputs for a future data-driven pricing model. This matches the cold-start, owner-set, transparent design set out in the proposal, and stops short of the machine-learning pricing model reserved for future work.
...
| NF6 | Modular, extensible architecture with audited decisions | Met | Eighteen self-contained modules; every verification decision persists the reviewer and reason and writes an audit-log row surfaced at an admin endpoint. |
...
Testing is layered deliberately: **unit tests** for the custom logic (mocked dependencies, fast and deterministic), a **service-level integration test** that boots the real Nest application against a real PostGIS database and drives the core flow over HTTP, **widget tests** for the mobile screens, a **manual end-to-end matrix** across two roles plus admin, and the **post-deploy verification** above. The unit suite concentrates on the logic where correctness matters most and a fast test is meaningful: the pricing formula and its rounding, floor, and fallback paths; the proposal accept/decline state machine; the authentication boundary; the verification access guard; job create-persistence and the owner-driven status lifecycle; the routing proxy's OSRM parse, server-side instruction composition, and great-circle fallback; and the pass-through payment guards including webhook idempotency. Above that, the integration test proves the guards hold across the real request path rather than only in mocks — it registers an owner and a driver, posts a job, sends and accepts a proposal, and asserts both that the job reaches `matched` and that counterparty contact details are hidden until the moment of acceptance. Geospatial distance is delegated to PostGIS and is therefore exercised through that live database path rather than by re-implementing and unit-testing the maths.
...
| --- | --- | --- |
| API unit suite (Jest) | pricing v3 formula (distance + time + **weight term**, min-fare floor, fallback, weight-sensitivity); proposal accept/decline state machine; auth boundary; verification access guard + **go-online nudge on approval**; **job create-persistence + status lifecycle**; **routing OSRM parse + great-circle fallback + instruction composition**; **pass-through payments guards + webhook idempotency**; **SMS provider (stub/real, never-throws)** | 67 tests across 10 suites, all passing |
| API integration test (Jest + supertest) | boots a real Nest app against a real PostGIS database and drives register → post job → propose → accept over HTTP; asserts the job reaches `matched` and that counterparty contact is hidden until acceptance (the privacy gate) | 1 test, passing |
| Admin unit suite (Vitest) | directory pagination query builder; metrics formatters (null renders as "No data yet") | 9 tests across 2 suites, all passing |
| Mobile suite (Flutter) | model JSON round-trip, screen render/input, widget tests, driver job-detail (shows estimate + posted price), navigation route-follower geometry, local-first logout | 20 tests across 12 files, all passing |
...
**A defect that the integration test surfaced, and how it was resolved.** Not every test passed on the first attempt, and one failure is worth recording because it is exactly the kind of problem an integration test exists to catch. When the new integration test was first run on its own it passed, but running it as part of the full API suite produced a red result: `expected 201 "Created", got 409 "Conflict"` on the registration step (Figure X). The diagnosis was a **test-isolation defect** rather than a fault in the application. The first version of the test registered its owner and driver under fixed email addresses and did not clean up after itself, so the accounts it created were left in the test database; a second run of the same test then tried to register the same email again, and the `users` table's unique-email index correctly rejected it with a 409. In other words, the failure was in the test's own hygiene: it was not idempotent. The fix was to make each run self-contained — the test now generates unique credentials per run (a timestamp-plus-random suffix on the email and phone) and deletes the two accounts it created in an `afterAll` teardown, so repeated runs leave no residue. After the fix the full suite passes consistently on repeated runs (Figure Y). This episode is a small but genuine illustration of why the integration layer earns its place: the underlying privacy-gate and state-machine assertions were sound, but only running the test the way it would actually be run — repeatedly, alongside everything else, against a real database — exposed the isolation flaw, which a purely mocked unit test could not have shown.
...
Second, **automated test coverage is uneven, though it now spans both unit and integration levels.** It is strong on the pieces most likely to be wrong in a subtle way — pricing, the proposal state machine, auth, verification access, the job lifecycle, routing and its fallback, and the payment guards — and a single integration test exercises the core register-to-match flow against a real database over HTTP. What remains uncovered by fast automated checks: a few domain services (matching's PostGIS query, ratings, messaging) still rely on the manual matrix and live integration rather than dedicated tests, and the mobile suite is mostly render-level smoke tests plus two behavioural suites (the navigation route-follower and the local-first logout) rather than broad widget-behaviour coverage. Extending the integration layer to more flows (payment settlement, rating after completion) and adding matching/ratings unit tests are the natural next steps.
...
Fourth, **the seeded pricing parameters are placeholders, not field-researched rates.** The pricing rule, its distance and time terms, and its per-vehicle floors are all in place and configurable, but the seeded values are indicative rather than calibrated against real Kigali rates. Because the parameters live in a configuration table and are editable without a redeploy, calibrating them is a data task rather than a code change; it depends on the same field data that the pending user study would gather. (The earlier limitation of a straight-line-only distance has since been resolved: the M7 increment replaced it with OSRM road routing, keeping the great-circle result only as a fallback.)
...
Sixth, **the app can be unreachable from one Rwandan mobile network because of a carrier-to-host routing problem, not a defect in Loop.** Discovered during on-device testing on 10 July 2026, late in the project, the app timed out at connection on MTN Rwanda mobile data while working normally on Wi-Fi and on other networks. It surfaced late precisely because it only affects specific mobile-data paths: the emulators, the development network, and Wi-Fi all reached the API normally, so nothing earlier exposed it. Investigation showed the API itself was healthy (it returned HTTP 200 within about two seconds when reached through an external proxy that takes a different network path), but the single edge IP that the hosting provider's domain resolves to was not routable from that carrier — a peering or routing blackhole to that specific address, which no code change can fix. This is a documented pattern with the hosting provider (Railway) rather than something unique to this project: the provider's staff have confirmed on their own forum that the same symptom originates on the mobile carrier's side and recommend the same CDN fix (reported there for the Jio network in India; the mechanism matches the MTN case here). It matters for the evaluation because participants may be on mobile data. The durable remedy is to stop depending on that single provider edge IP by fronting the API with a custom domain behind a CDN (Cloudflare) whose edge the local carriers reach reliably; the CDN reaches the host from its own healthy path. This is a configuration change in keeping with the deployment posture (no code rewrite), and the step-by-step plan and the supporting references are in the deployment guide (Section 4.3 and DEPLOYMENT.md section 12).
...
**Making payments pass-through, rather than held, was a scoping decision with real weight.** The literature review found that the regional platforms that struggled were precisely the ones carrying capital: paying drivers upfront while waiting on shippers. Loop provides in-app payment but never holds, escrows, or fronts the funds — the money moves owner → provider → driver and Loop only initiates and records the verified outcome — so it sidesteps that failure mode entirely and stays light enough to run on a pilot budget. This is a deliberate boundary (no escrow, no commission), and it keeps the trust-and-matching core at the centre of the product rather than turning Loop into a financial institution.
...
- **Traffic-aware routing (building on the road routing now in place).** Road routing and in-app turn-by-turn navigation, once listed here as future work, were delivered in the M7 increment using OSRM: the price estimate uses road distance and duration, and the driver has full in-app turn-by-turn (route line, follow-me camera, voice guidance, off-route rerouting), keeping the maps-app deep link as a secondary "Open in Maps" option. This stayed within the "no key, no Google" constraint, since OSRM is OpenStreetMap-native and self-hostable. What remains as future work is *traffic-aware* routing: OSRM's routes are free-flow and ignore congestion, so route times and alternate-route suggestions that reflect live traffic would require a metered, key-gated provider (Google, Mapbox, HERE, or a self-hosted engine fed a traffic dataset). That is a deliberate reversal of the no-key stance and a recurring cost, so it should be taken on only if field use shows drivers routinely hitting congestion the free-flow route cannot see.
...
- **In-app payments — built as pass-through; extend, don't add escrow.** Pass-through in-app payment is implemented (a swappable `PAYMENT_DRIVER`: stub, Flutterwave v3 hosted card + MTN Mobile Money, or Flutterwave v4 Mobile Money), with the verified provider webhook as the sole source of truth and the charge locked to the posted price. The remaining work is real-money go-live (Rwanda merchant onboarding / KYC) and broadening payment methods — deliberately **without** escrow, because carrying capital is exactly what strained comparable platforms; the design adds convenience without taking on settlement risk.
