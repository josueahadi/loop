...
**Specific objective 2: design and develop the MVP across iterative cycles.** Fully addressed. The MVP was built in milestones (M1 to M6, plus an M3.5 location increment), each delivering a working slice: foundation and auth, matching, pricing and jobs, location and geocoding, the transaction loop, trust and ratings, and the admin console. A later M7 increment added road routing, a distance-and-time pricing rule, and in-app turn-by-turn navigation (see [Section 5](#5-discussion)). Every functional requirement is implemented ([Section 4.1](#41-functional-requirements-a-traceability-view)).
...
**The pricing rule is rule-based and transparent.** The suggested price combines a per-vehicle-type floor, a base fare, a distance term, and a time term, all scaled by a load-size factor:
...
It is computed in the pricing service and rounded to whole Rwandan francs (the currency has no minor unit, so amounts are integers and are never multiplied by a hundred). The `min_fare` floor sits inside the `max()`, so the size factor scales the floor too, and a very short trip still covers a minimum. The distance and duration come from road routing (an OSRM proxy); when the router is unavailable the estimate falls back to the PostGIS great-circle distance, drops the time term, and flags the source, so an estimate is never blocked on a third party. Every parameter is read from a configuration table seeded from requirements data, not hard-coded, so it can be corrected without a redeploy.

The seeded parameters (placeholders pending field research, all in whole RWF) are:
...
The load-size factor is `1.0` for small, `1.3` for medium, and `1.6` for large. As a worked example, a pickup carrying a medium load over a 10 km / 20 min road trip is `max(1500, 1000 + 600×10 + 60×20) × 1.3 = max(1500, 8200) × 1.3 = 10,660 RWF`.
...
| --- | --- | --- |
| API unit suite (Jest) | pricing formula (distance + time, min-fare floor, fallback); proposal accept/decline state machine; auth boundary; verification access guard; **job create-persistence + status lifecycle**; **routing OSRM parse + great-circle fallback + instruction composition**; **pass-through payments guards + webhook idempotency** | 47 tests across 7 suites, all passing |
...
| Mobile suite (Flutter) | model JSON round-trip, screen render/input, widget tests, navigation route-follower geometry, local-first logout | 18 tests across 11 files, all passing |
