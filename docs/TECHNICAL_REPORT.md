...
| --- | --- | --- |
| API unit suite (Jest) | pricing formula (distance + time, min-fare floor, fallback), proposal accept/decline state machine, auth boundary, verification access guard | 24 tests across 4 suites, all passing |
...
| Mobile suite (Flutter) | model JSON round-trip, screen render/input, widget tests, navigation route-follower geometry, local-first logout | 11 test files, passing |
