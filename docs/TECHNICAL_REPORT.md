...
| --- | --- | --- |
| API unit suite (Jest) | pricing v3 formula (distance + time + **weight term**, min-fare floor, fallback, weight-sensitivity); proposal accept/decline state machine; auth boundary; verification access guard + **auto-activate-on-approval**; **job create-persistence + status lifecycle**; **routing OSRM parse + great-circle fallback + instruction composition**; **pass-through payments guards + webhook idempotency**; **SMS provider (stub/real, never-throws)** | 58 tests across 8 suites, all passing |
